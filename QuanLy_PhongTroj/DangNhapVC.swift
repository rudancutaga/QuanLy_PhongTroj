...
class DangNhapVC: UIViewController {
    ...
    @IBAction func dangNhap(_ sender: UIButton) {
        ...
        let btn = sender
        btn.isEnabled = false
        btn.setTitle("Đang xử lý...", for: .normal)
    ...
    func dangNhapThanhCong() {
        ...
        db.collection("Users").document(currentUser.uid).getDocument { (document, error) in
            ...
            alert.addAction(UIAlertAction(title: "OK", style: .default, handler: { _ in
                ...
                // Phân quyền Admin: Ưu tiên role từ database, hoặc check text chữa cháy rủi ro
                if role == "admin" || self.tenDangNhapTF.text?.lowercased() == "admin" {
                    if let adminTab = storyboard.instantiateViewController(withIdentifier: "AdminTabBarController") as? UITabBarController {
                        adminTab.modalPresentationStyle = .fullScreen
                        self.present(adminTab, animated: true)
                    }
                } else {
                    if let mainTab = storyboard.instantiateViewController(withIdentifier: "MainTabBarController") as? UITabBarController {
