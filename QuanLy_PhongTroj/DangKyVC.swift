...
class DangKyVC: UIViewController {
    ...
    @IBAction func dangKy(_ sender: UIButton) {
        ...
        setLoading(true, title: "Đang chờ...")
        ...
        Auth.auth().createUser(withEmail: emailDangKy, password: matKhau) { authResult, error in
            ...
            self.setLoading(false, title: "ĐĂNG KÝ")
    ...
    private func setLoading(_ loading: Bool, title: String) {
        dangKyBT.isEnabled = !loading
        if dangKyBT.configuration != nil {
            dangKyBT.configuration?.title = title
        } else {
            dangKyBT.setTitle(title, for: .normal)
        }
    }
