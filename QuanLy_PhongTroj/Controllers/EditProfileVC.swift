...
class EditProfileVC: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    @IBAction func saveTapped(_ sender: UIButton) {
        ...
        if saveBtn.configuration != nil {
            saveBtn.configuration?.title = "Đang lưu..."
        } else {
    ...
    private func updateFirestore(uid: String, name: String, phone: String, avatarUrl: String? = nil) {
        ...
        Firestore.firestore().collection("Users").document(uid).setData(values, merge: true) { [weak self] err in
            self?.saveBtn.isEnabled = true
            if self?.saveBtn.configuration != nil {
                self?.saveBtn.configuration?.title = "Cập nhật"
            } else {
